#!/usr/bin/env python3
"""Serve Elevation over HTTPS on all interfaces for Mac + iOS LAN preview.
Geolocation and secure APIs need a secure context on device browsers."""
...
def main():
    ...
    if mode == "https":
        ...
        print("Elevation HTTPS", flush=True)
        print(f"  Mac:    https://127.0.0.1:{port}/", flush=True)
        print(f"  iOS:    https://{lan_hint}:{port}/", flush=True)
        print("  Self-signed: tap Advanced → proceed on first visit.", flush=True)
    else:
        print("Elevation HTTP", flush=True)
        print(f"  Mac:    http://127.0.0.1:{port}/", flush=True)
        print(f"  iOS:    http://{lan_hint}:{port}/", flush=True)
